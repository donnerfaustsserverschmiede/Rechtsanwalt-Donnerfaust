...
(() => {
  ...
  async function share(id){
    ...
    const snapshot={version:5,invoice:{invoice_number:i.invoice_number,date:i.date,status:i.status,amount:i.amount,lines:i.lines||[],billing_recipient:i.billing_recipient||{name:"Thorson Donnerfaust",account:"4693"},employee:e?{name:e.name,role:e.role}:null},client:c?{name:c.name,email:c.email,address:c.address}:null,case:a?{file_number:a.file_number,title:a.title}:null};
    for(let n=0;n<3;n++){const t=token();let r=await cloud.from("shared_invoice_access").insert({token:t,invoice_number:i.invoice_number,snapshot,active:true});if(!r.error){openLink(i.invoice_number,t);return;}}
    /* Fallback: Die bestehende Akten-Freigabetabelle kann ebenfalls schreibgeschützte Snapshots tragen. */
    for(let n=0;n<3;n++){const t=token();let r=await cloud.from("shared_case_access").insert({token:t,file_number:i.invoice_number,snapshot,active:true});if(!r.error){openLink(i.invoice_number,t);return;}}
    alert("Der Rechnungslink konnte nicht erstellt werden. Die gemeinsame Supabase-Freigabe ist nicht erreichbar.");
  }
