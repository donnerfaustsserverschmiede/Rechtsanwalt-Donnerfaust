...
(() => {
  ...
  const write = d => {
    ...
    try { window.dispatchEvent(new Event("donnerfaust-data-changed")); } catch {}
  };
  ...
  const makeLink = (email, name, role) => {
    ...
    u.searchParams.set("einladung", data);
    return u.toString();
  };
  ...
  function openForm(emp) {
    ...
    box.querySelector("#dfemx").onclick = box.querySelector("#dfemcancel").onclick = close;
    box.querySelector("#dfemform").onsubmit = async e => {
      e.preventDefault();
      const f = new FormData(e.target), d = read(); d.employees ||= [];
      const name = String(f.get("name") || "").trim(), email = String(f.get("email") || "").trim(), role = normalizeRole(String(f.get("role") || "")), status = String(f.get("status") || "Online");
      if (emp) {
        const target = d.employees.find(x => String(x.id) === String(emp.id));
        if (target) Object.assign(target, { name, email, role, status });
      } else {
        d.employees.push({ id: uid(), name, email, role, status });
      }
      write(d);
      const link = makeLink(email, name, role);
      box.querySelector("#dfemresult").innerHTML = `<div style="margin-top:14px;padding:14px;border-radius:12px;background:#f3f4f6"><b>Einladungslink</b><textarea id="dfemlink" readonly style="width:100%;height:78px;box-sizing:border-box;margin-top:8px;padding:9px">${esc(link)}</textarea><button type="button" class="btn dark" id="dfemcopy" style="width:100%;margin-top:8px">🔗 Link kopieren</button><div id="dfemcopymsg" style="margin-top:7px;font-size:13px"></div></div>`;
  ...
  async function removeEmployee(id) {
    ...
    if (!e) return;
    if (String(id) === "owner") { alert("Der Kanzleiinhaber kann nicht gelöscht werden."); return; }
    if (!confirm(`Mitarbeiter „${e.name || ""}“ wirklich löschen?`)) return;
    d.employees = d.employees.filter(x => String(x.id) !== String(id));
    write(d); location.reload();
  }
  function normalizeEmployees() {
    const d = read(); if (!Array.isArray(d.employees)) return;
    let changed = false;
    d.employees.forEach(e => { const r = normalizeRole(e.role); if (e.role !== r) { e.role = r; changed = true; } });
    if (changed) write(d);
  }
  function renderPanel() {
    const content = findContent(); if (!content) return;
    let panel = document.getElementById("dfEmployeeManagementPanel");
    if (!panel) {
      panel = document.createElement("div"); panel.id = "dfEmployeeManagementPanel"; panel.className = "panel"; panel.style.marginTop = "18px";
      content.appendChild(panel);
    }
    const d = read(); d.employees ||= [];
    panel.innerHTML = `<div class="panelhead"><div><b>Mitarbeiterverwaltung</b><small style="display:block">Aufgabe und Vergütung werden über die Rolle zugeordnet.</small></div><button type="button" class="btn dark" id="dfemnew">+ Mitarbeiter</button></div><div style="display:grid;gap:10px;margin-top:12px">${d.employees.map(e => `<div class="listrow" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap"><div style="flex:1;min-width:180px"><b>${esc(e.name || "Unbenannt")}</b><small style="display:block">${esc(e.email || "Keine E-Mail")} · ${esc(normalizeRole(e.role))}</small></div><span class="badge ${e.status === "Offline" ? "warn" : "good"}">${esc(e.status || "Online")}</span><button type="button" class="mini" data-df-edit="${esc(e.id)}">Bearbeiten</button><button type="button" class="mini" data-df-delete="${esc(e.id)}">Löschen</button></div>`).join("")}</div>`;
  ...
  function interceptOldButton() {
    document.querySelectorAll('[data-action="newemployee"]').forEach(b => {
      if (b.dataset.dfManaged) return;
      b.dataset.dfManaged = "1";
      b.addEventListener("click", e => { e.preventDefault(); e.stopImmediatePropagation(); openForm(null); }, true);
    });
  }
